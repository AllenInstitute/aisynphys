...
from aisynphys.connectivity import CorrectionModel, BinaryModel, ErfModel, ei_correct_connectivity
...
def data_snippets(conntype='finite'):
    ...
    # pre_cell type
    cell = lambda: None # initiating a function so that I can add an attribute to it
...
def test_correction_model():
    ...
    correction_parameters = [[np.array([6.939e-2, 2.000e-4, 6.003e-1]),
                              np.array([6.893e-2, 4.021e-5, 2.848e-5]),
    ...
    assert corr_model.size == 100e-6

    fit = CorrectionModel.fit(corr_model, variables, conn, excinh=1)
    assert corr_model.pmax == pytest.approx(0.74590713, 1e-4)
    assert fit.x == pytest.approx(0.74590713, 1e-4)
    assert corr_model.likelihood(variables, conn) == pytest.approx(-1.950749092021, 1e-4)
    assert fit.fun == pytest.approx(1.950749092021, 1e-4)
    
    assert fit.cp_ci[0] == corr_model.pmax
    assert fit.cp_ci[1] == pytest.approx(0.04785445, 1e-4) # lower MINOS value
    assert fit.cp_ci[2] == pytest.approx(2.17707071, 1e-4) # upper MINOS value
    ...
    assert fit.cp_ci[2] == pytest.approx(1.031550298, 1e-4) # upper MINOS value
    ...
    assert fit.cp_ci[1] == pytest.approx(1.8731900, 1e-4) # lower MINOS value
